...
- **A done task is a drawn green check, not a greyed-out box** (#166). The engine emits the checkbox `disabled` - it is a rendered state, not a control - and a disabled box is painted by the browser in its own grey with `accent-color` ignored, so both states came out the same washed square. The box is drawn now: a bordered empty square, or a filled success square with a tick. The label is left readable, which also removes the `:not(:has(:is(ul, ol)))` carve-out the old strike needed to stop a finished parent striking its open children.

## [0.1.3] - 2026-08-27

### Added

- **The preview is styled by `@markup-carve/carve-css` 0.1.0** (#165). It hand-rolled its own stylesheet before, which is why it looked unlike the IntelliJ preview: the two were separately maintained approximations of the same design. The four layers - tokens, core, extensions and recipes - are bundled into `media/` and linked, and the scaffold now holds no color literal at all. Every `--carve-*` token is bound to the `--vscode-*` variable that means the same thing, so the document is themed by whatever theme the editor is wearing, and re-themed live when it changes, because VS Code rewrites those variables in place.

- **The seven documented recipe families render** (#165). `::: tree`, `::: cards`, `::: columns`, `::: steps`, `::: aside`, `{.lead}` with `{.badge}`, and the table modifiers `{.striped}` / `{.compact}` / `{.ok}` / `{.warn}` all carried their classes into the preview and met no CSS, so a tree was a plain list and a striped table a plain table. `recipes.css` is bundled and the preview body carries the `.carve` scope class every rule in the package keys off.

- **A code fence carries a copy button and shows its language at rest** (#165). The language pill existed but was `opacity: 0` until the pointer was over the block, so nothing said what a fence held; there was no copy button at all. Both now sit in a tool strip beside the code at 0.45, solid on hover or keyboard focus. The strip is a sibling of the `<pre>`, not a child, because a `<pre>` scrolls its own content and a child pinned to its corner slides out of view on exactly the long lines a reader wants to copy. The copy goes through the extension host and `vscode.env.clipboard`: a webview is an iframe without the `clipboard-write` permission policy, so `navigator.clipboard.writeText` rejects there and is kept only as the fallback for the exported HTML. Both outcomes are reported on the button.

- **Four more extensions are enabled: citations, wikilinks, autolink and external links** (#165). The preview enabled eight where the docs playground enables thirteen, and nothing kept the two in step. `citations` is the one that mattered: it changes the parse rather than the render, so `[@key]` produced no citation node at all and the whole construct was unreachable here while it worked in the playground. `headingPermalinks` stays off deliberately - the paragraph anchors are noise in an editor preview, which nobody deep-links into - and that decision is recorded next to the list so it is not closed as a gap. Every extension the pinned engine exports must now be classified as enabled or excluded-with-a-reason, which is what stops the set drifting again.
...
- **A code block is one flat surface instead of a white slab** (#165). The highlight.js themes paint the inner element themselves - `.hljs { background: #fff; padding: 1em }` - so a hardcoded white rectangle with its own padding sat inside the `pre`'s themed padding. In a light theme that was an inset frame; in a dark theme the code block was a white slab, which made it a dark-mode defect rather than a cosmetic one. carve-css resets the inner element, and the reset wins because the highlight themes are linked ahead of it. The link order is now pinned by a test.

- **A tab set and a code group show one panel and switch between them** (#165). They had CSS but no behavior: every panel rendered stacked and the tabs were inert labels. No script was needed - the extensions emit the CSS-only shape, a hidden radio per tab with a bound label - only a selector matching the order the engine emits, which is all the controls and then all the panels. carve-css 0.1.0 pairs a panel with the label immediately before it, which against that order matches nothing on the first tab and the *first* panel on the second, so selecting tab two revealed tab one. Panels are now paired with their own radio by position, and the radios stay focusable so the construct is keyboard reachable.

- **A finished task item is tellable from an unfinished one** (#165). Both states rendered as the same greyed box, because the engine emits the checkbox disabled and nothing distinguished them further. A done item's label now goes soft and struck. The strike is withheld from an item carrying a nested list, since a text decoration set on a block box propagates into every descendant and CSS gives the descendant no way to take it back - a finished parent does not finish its children.

- **The preview follows a theme switch while it is open** (#165). Color follows on its own now that the tokens are bound to the editor's variables, but the highlight.js stylesheet and mermaid are not CSS: mermaid bakes its palette into the SVG it has already emitted. Both are re-applied when the editor theme changes, so code and diagrams no longer keep the old theme until something else forces a re-render. High contrast is treated explicitly - the borders take the theme's contrast border, and the tints stay, because in `tr.ok` versus `tr.warn` and in a toned badge the tint is the only signal there is.
